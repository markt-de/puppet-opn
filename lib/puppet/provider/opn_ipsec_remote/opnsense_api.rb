...
Puppet::Type.type(:opn_ipsec_remote).provide(:opnsense_api) do
  desc 'Manages OPNsense IPsec remote authentication entries via the REST API.'
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('ipsec/connections/searchRemote', {})
      rows     = response['rows'] || []
      ...
      Puppet.warning("opn_ipsec_remote: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  def create
    ...
    result = client.post('ipsec/connections/addRemote', { 'remote' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_ipsec_remote: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("ipsec/connections/delRemote/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_ipsec_remote: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("ipsec/connections/setRemote/#{uuid}", { 'remote' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_ipsec_remote: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
