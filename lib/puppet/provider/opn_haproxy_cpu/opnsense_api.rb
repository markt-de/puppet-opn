...
Puppet::Type.type(:opn_haproxy_cpu).provide(:opnsense_api) do
  desc 'Manages OPNsense HAProxy CPU affinity via the REST API.'
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('haproxy/settings/search_cpus', {})
      rows     = response['rows'] || []
      ...
      Puppet.warning("opn_haproxy_cpu: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  # Called once after ALL opn_haproxy_cpu resources are evaluated.
  ...
  def create
    ...
    result = client.post('haproxy/settings/add_cpu', { 'cpu' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_haproxy_cpu: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("haproxy/settings/del_cpu/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_haproxy_cpu: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("haproxy/settings/set_cpu/#{uuid}", { 'cpu' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_haproxy_cpu: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
