...
Puppet::Type.type(:opn_haproxy_user).provide(:opnsense_api) do
  desc 'Manages OPNsense HAProxy user-list users via the REST API.'
  ...
  # Called once after ALL opn_haproxy_user resources are evaluated.
  ...
  def create
    ...
    result = client.post('haproxy/settings/add_user', { 'user' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_haproxy_user: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("haproxy/settings/del_user/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_haproxy_user: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("haproxy/settings/set_user/#{uuid}", { 'user' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_haproxy_user: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
