...
Puppet::Type.type(:opn_kea_dhcpv6_peer).provide(:opnsense_api) do
  desc 'Manages OPNsense KEA DHCPv6 HA peers via the REST API.'

  extend  PuppetX::Opn::ProviderBase::ClassMethods
  include PuppetX::Opn::ProviderBase::InstanceMethods
  reconfigure_group :kea

  # Delegates reconfigure to ServiceReconfigure after all opn_kea_dhcpv6_peer
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('kea/dhcpv6/searchPeer', {})
      rows     = response['rows'] || []
      ...
      Puppet.warning("opn_kea_dhcpv6_peer: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  def create
    ...
    result = client.post('kea/dhcpv6/addPeer', { 'peer' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_kea_dhcpv6_peer: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("kea/dhcpv6/delPeer/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_kea_dhcpv6_peer: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("kea/dhcpv6/setPeer/#{uuid}", { 'peer' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_kea_dhcpv6_peer: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
