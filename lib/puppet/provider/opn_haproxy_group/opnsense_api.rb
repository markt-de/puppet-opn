...
Puppet::Type.type(:opn_haproxy_group).provide(:opnsense_api) do
  desc 'Manages OPNsense HAProxy user-list groups via the REST API.'
  ...
  def self.relation_fields
    {
      'members' => { endpoint: 'haproxy/settings/search_users', multiple: true },
    }.freeze
  end
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      begin
        ...
        response = client.post('haproxy/settings/search_groups', {})
        rows     = response['rows'] || []
        ...
        Puppet.warning("opn_haproxy_group: failed to fetch from '#{device_name}': #{e.message}")
      end
    end
  ...
  # Called once after ALL opn_haproxy_group resources are evaluated.
  ...
  def create
    ...
    result = client.post('haproxy/settings/add_group', { 'group' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_haproxy_group: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("haproxy/settings/del_group/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_haproxy_group: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("haproxy/settings/set_group/#{uuid}", { 'group' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_haproxy_group: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
