...
Puppet::Type.type(:opn_kea_dhcpv6_pd_pool).provide(:opnsense_api) do
  desc 'Manages OPNsense KEA DHCPv6 prefix delegation pools via the REST API.'
  ...
  # IdResolver translates between UUIDs and subnet CIDRs on write operations.
  ...
  # Delegates reconfigure to ServiceReconfigure after all opn_kea_dhcpv6_pd_pool
  # resources have been evaluated in this catalog run.
  def self.post_resource_eval
    PuppetX::Opn::ServiceReconfigure[:kea].run
  end

  # Uses search-only pattern: searchPdPool returns ALL fields.
  # The subnet field in search results is already a display value (CIDR),
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('kea/dhcpv6/searchPdPool', {})
      rows     = response['rows'] || []
      ...
      Puppet.warning("opn_kea_dhcpv6_pd_pool: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  def create
    ...
    result = client.post('kea/dhcpv6/addPdPool', { 'pd_pool' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_kea_dhcpv6_pd_pool: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("kea/dhcpv6/delPdPool/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_kea_dhcpv6_pd_pool: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("kea/dhcpv6/setPdPool/#{uuid}", { 'pd_pool' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_kea_dhcpv6_pd_pool: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
