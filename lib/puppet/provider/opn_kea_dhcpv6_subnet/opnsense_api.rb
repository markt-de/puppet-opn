...
Puppet::Type.type(:opn_kea_dhcpv6_subnet).provide(:opnsense_api) do
  desc 'Manages OPNsense KEA DHCPv6 subnets via the REST API.'
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('kea/dhcpv6/searchSubnet', {})
      rows     = response['rows'] || []

      rows.each do |row|
        uuid = row['uuid']
        # Fetch full subnet details (option_data, pools, allocator, etc.)
        detail    = client.get("kea/dhcpv6/getSubnet/#{uuid}")
        item_data = normalize_config(detail['subnet6'] || {})
      ...
      Puppet.warning("opn_kea_dhcpv6_subnet: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  def create
    ...
    result = client.post('kea/dhcpv6/addSubnet', { 'subnet6' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_kea_dhcpv6_subnet: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("kea/dhcpv6/delSubnet/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_kea_dhcpv6_subnet: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("kea/dhcpv6/setSubnet/#{uuid}", { 'subnet6' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_kea_dhcpv6_subnet: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
