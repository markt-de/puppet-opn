...
Puppet::Type.type(:opn_zabbix_agent_userparameter).provide(:opnsense_api) do
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      items    = response.dig('zabbixagent', 'userparameters', 'userparameter') || {}
      ...
      Puppet.warning(
        "opn_zabbix_agent_userparameter: failed to fetch from '#{device_name}': #{e.message}",
      )
    end
  ...
  def create
    ...
    result = client.post('zabbixagent/settings/addUserparameter', { 'userparameter' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_zabbix_agent_userparameter: failed to create '#{item_key}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("zabbixagent/settings/delUserparameter/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_zabbix_agent_userparameter: failed to delete '#{item_key}' " \
  ...
  def flush
    ...
    result = client.post(
      "zabbixagent/settings/setUserparameter/#{uuid}",
      { 'userparameter' => config },
    )
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_zabbix_agent_userparameter: failed to update '#{item_key}' " \
            "(uuid: #{uuid}): #{result.inspect}"
    end
