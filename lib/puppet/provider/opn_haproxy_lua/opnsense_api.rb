...
Puppet::Type.type(:opn_haproxy_lua).provide(:opnsense_api) do
  desc 'Manages OPNsense HAProxy Lua scripts via the REST API.'
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.post('haproxy/settings/search_luas', {})
      rows     = response['rows'] || []
      ...
      Puppet.warning("opn_haproxy_lua: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  # Called once after ALL opn_haproxy_lua resources are evaluated.
  ...
  def create
    ...
    result = client.post('haproxy/settings/add_lua', { 'lua' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_haproxy_lua: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("haproxy/settings/del_lua/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_haproxy_lua: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("haproxy/settings/set_lua/#{uuid}", { 'lua' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_haproxy_lua: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
