...
Puppet::Type.type(:opn_haproxy_frontend).provide(:opnsense_api) do
  ...
  # Called once after ALL opn_haproxy_frontend resources are evaluated.
  ...
  def create
    ...
    result = client.post('haproxy/settings/add_frontend', { 'frontend' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error, "opn_haproxy_frontend: failed to create '#{item_name}': #{result.inspect}"
    end
  ...
  def destroy
    ...
    result = client.post("haproxy/settings/del_frontend/#{uuid}", {})
    unless result['result'].to_s.strip.downcase == 'deleted'
      raise Puppet::Error,
            "opn_haproxy_frontend: failed to delete '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
  ...
  def flush
    ...
    result = client.post("haproxy/settings/set_frontend/#{uuid}", { 'frontend' => config })
    unless result['result'].to_s.strip.downcase == 'saved'
      raise Puppet::Error,
            "opn_haproxy_frontend: failed to update '#{item_name}' (uuid: #{uuid}): #{result.inspect}"
    end
