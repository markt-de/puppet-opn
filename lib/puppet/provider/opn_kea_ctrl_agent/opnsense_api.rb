...
Puppet::Type.type(:opn_kea_ctrl_agent).provide(:opnsense_api) do
  desc 'Manages OPNsense KEA Control Agent settings via the REST API.'
  ...
  # Delegates reconfigure to ServiceReconfigure after all opn_kea_ctrl_agent
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.get('kea/ctrl_agent/get')
      data     = response['ctrlagent'] || {}
      ...
      Puppet.warning("opn_kea_ctrl_agent: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  # Save Control Agent settings via the set endpoint
  def save_settings(client, config)
    result = client.post('kea/ctrl_agent/set', { 'ctrlagent' => config })
    return if result['result'].to_s.strip.downcase == 'saved'
    raise Puppet::Error,
          "opn_kea_ctrl_agent: failed to save settings for '#{resource[:name]}': #{result.inspect}"
  end
