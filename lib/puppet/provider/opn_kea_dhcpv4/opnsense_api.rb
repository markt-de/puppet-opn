...
Puppet::Type.type(:opn_kea_dhcpv4).provide(:opnsense_api) do
  desc 'Manages OPNsense KEA DHCPv4 global settings via the REST API.'
  ...
  # Delegates reconfigure to ServiceReconfigure after all opn_kea_dhcpv4
  ...
  def self.instances
    ...
    PuppetX::Opn::ApiClient.device_names.each do |device_name|
      ...
      response = client.get('kea/dhcpv4/get')
      data     = response['dhcpv4'] || {}

      # Extract only the managed sections and normalize selection hashes
      # (interfaces, dhcp_socket_type in general section)
      ...
      Puppet.warning("opn_kea_dhcpv4: failed to fetch from '#{device_name}': #{e.message}")
    end
  ...
  # Save DHCPv4 settings via the set endpoint
  def save_settings(client, config)
    result = client.post('kea/dhcpv4/set', { 'dhcpv4' => config })
    return if result['result'].to_s.strip.downcase == 'saved'
    raise Puppet::Error,
          "opn_kea_dhcpv4: failed to save settings for '#{resource[:name]}': #{result.inspect}"
  end
