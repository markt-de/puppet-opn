...
Puppet::Type.newtype(:opn_haproxy_healthcheck) do
  desc <<-DOC
    ...
    @example Define an HTTP health check
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "healthcheck_name@device_name" format.

      The "healthcheck_name" portion (before "@") is the identifier used in
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of health check configuration options passed directly to the OPNsense API.
