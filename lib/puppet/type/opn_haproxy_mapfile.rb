...
Puppet::Type.newtype(:opn_haproxy_mapfile) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "mapfile_name@device_name" format.

      The "mapfile_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of map file configuration options passed directly to the OPNsense API.
