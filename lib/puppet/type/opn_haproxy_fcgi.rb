...
Puppet::Type.newtype(:opn_haproxy_fcgi) do
  desc <<-DOC
    ...
    @example Define a FastCGI application
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "fcgi_name@device_name" format.

      The "fcgi_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of FastCGI application configuration options passed directly to the OPNsense API.
