...
Puppet::Type.newtype(:opn_haproxy_acl) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "acl_name@device_name" format.

      The "acl_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of ACL configuration options passed directly to the OPNsense API.
