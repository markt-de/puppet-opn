...
Puppet::Type.newtype(:opn_haproxy_errorfile) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "errorfile_name@device_name" format.

      The "errorfile_name" portion (before "@") is the identifier used in
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of error file configuration options passed directly to the OPNsense API.
