...
Puppet::Type.newtype(:opn_haproxy_action) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "action_name@device_name" format.

      The "action_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of action configuration options passed directly to the OPNsense API.
