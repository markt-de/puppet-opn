...
Puppet::Type.newtype(:opn_haproxy_resolver) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "resolver_name@device_name" format.

      The "resolver_name" portion (before "@") is the identifier used in
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of DNS resolver configuration options passed directly to the OPNsense API.
