...
Puppet::Type.newtype(:opn_haproxy_lua) do
  desc <<-DOC
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "lua_name@device_name" format.

      The "lua_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of Lua script configuration options passed directly to the OPNsense API.
