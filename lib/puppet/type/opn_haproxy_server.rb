...
Puppet::Type.newtype(:opn_haproxy_server) do
  desc <<-DOC
    ...
    @example Define a backend server
  ...
  newparam(:name, namevar: true) do
    desc <<-DOC
      The resource title in "server_name@device_name" format.

      The "server_name" portion (before "@") is the identifier used in the
  ...
  newproperty(:config) do
    desc <<-DOC
      A hash of server configuration options passed directly to the OPNsense API.
      ...
      Commonly used keys:
        ...
        enabled     - Whether the server is enabled (1 or 0)
