...
module PuppetX # rubocop:disable Style/ClassAndModuleChildren
  module Opn
    # Shared module used by all opn_ipsec_* providers to coordinate
    ...
    module IpsecReconfigure
      @devices_to_reconfigure = {}

      # Registers a device as having pending IPsec changes.
      ...
      # Called once after ALL opn_ipsec_* resources are evaluated.
      ...
      def self.run
        @devices_to_reconfigure.each do |device_name, client|
          reconf = client.post('ipsec/service/reconfigure', {})
          status = reconf.is_a?(Hash) ? reconf['status'].to_s.strip.downcase : nil
          if status == 'ok'
            Puppet.notice("opn_ipsec: reconfigure of '#{device_name}' completed")
          else
            Puppet.warning(
              "opn_ipsec: reconfigure of '#{device_name}' returned unexpected " \
              "status: #{reconf.inspect}",
            )
          end
        rescue Puppet::Error => e
          Puppet.err("opn_ipsec: reconfigure of '#{device_name}' failed: #{e.message}")
        end
