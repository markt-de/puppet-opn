...
module PuppetX # rubocop:disable Style/ClassAndModuleChildren
  module Opn
    # Shared module used by all opn_openvpn_* providers to coordinate
    ...
    module OpenvpnReconfigure
      @devices_to_reconfigure = {}

      # Registers a device as having pending OpenVPN changes.
      ...
      # Called once after ALL opn_openvpn_* resources are evaluated.
      ...
      def self.run
        @devices_to_reconfigure.each do |device_name, client|
          reconf = client.post('openvpn/service/reconfigure', {})
          status = reconf.is_a?(Hash) ? reconf['status'].to_s.strip.downcase : nil
          if status == 'ok'
            Puppet.notice("opn_openvpn: reconfigure of '#{device_name}' completed")
          else
            Puppet.warning(
              "opn_openvpn: reconfigure of '#{device_name}' returned unexpected " \
              "status: #{reconf.inspect}",
            )
          end
        rescue Puppet::Error => e
          Puppet.err("opn_openvpn: reconfigure of '#{device_name}' failed: #{e.message}")
        end
