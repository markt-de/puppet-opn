...
require 'puppet/type/opn_haproxy_healthcheck'
require 'puppet/provider/opn_haproxy_healthcheck/opnsense_api'

RSpec.describe Puppet::Type.type(:opn_haproxy_healthcheck).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'returns instances from the API' do
      allow(client).to receive(:post).with('haproxy/settings/search_healthchecks', {}).and_return(
        'rows' => [
          { 'uuid' => 'aaa-bbb', 'name' => 'tcp-check', 'type' => 'tcp' },
        ],
      )

      instances = described_class.instances
      expect(instances.length).to eq(1)
      expect(instances[0].name).to eq('tcp-check@opnsense01')
      expect(instances[0].get(:config)).to eq('name' => 'tcp-check', 'type' => 'tcp')
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = Puppet::Type.type(:opn_haproxy_healthcheck).new(
        name: 'tcp-check@opnsense01',
        ensure: :present,
      )
      resources = { 'tcp-check@opnsense01' => resource }

      described_class.prefetch(resources)
      expect(resource.provider.name).to eq('tcp-check@opnsense01')
    end
  end
  ...
  describe '#create' do
    let(:resource) do
      Puppet::Type.type(:opn_haproxy_healthcheck).new(
        name: 'tcp-check@opnsense01',
        ensure: :present,
        config: { 'type' => 'tcp' },
      )
    ...
    it 'sends correct POST to add endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/add_healthcheck',
        { 'healthcheck' => { 'type' => 'tcp', 'name' => 'tcp-check' } },
  ...
  describe '#destroy' do
    ...
    it 'sends correct POST to del endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/del_healthcheck/aaa-bbb', {}
  ...
  describe '#flush' do
    ...
    let(:provider) do
      described_class.new(
        ...
        device: 'opnsense01',
        uuid: 'aaa-bbb',
        config: { 'name' => 'tcp-check', 'type' => 'tcp' },
      )
    end
    ...
    it 'sends correct POST to set endpoint' do
      provider.config = { 'type' => 'http' }

      expect(client).to receive(:post).with(
        'haproxy/settings/set_healthcheck/aaa-bbb',
        { 'healthcheck' => { 'type' => 'http', 'name' => 'tcp-check' } },
      ).and_return({ 'result' => 'saved' })
      allow(PuppetX::Opn::HaproxyReconfigure).to receive(:mark)
