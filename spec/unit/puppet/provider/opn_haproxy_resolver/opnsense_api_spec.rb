...
require 'puppet/type/opn_haproxy_resolver'
require 'puppet/provider/opn_haproxy_resolver/opnsense_api'

RSpec.describe Puppet::Type.type(:opn_haproxy_resolver).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'returns instances from the API' do
      allow(client).to receive(:post).with('haproxy/settings/searchresolvers', {}).and_return(
        'rows' => [
          { 'uuid' => 'aaa-bbb', 'name' => 'dns01', 'nameserver' => '8.8.8.8' },
        ],
      )

      instances = described_class.instances
      expect(instances.length).to eq(1)
      expect(instances[0].name).to eq('dns01@opnsense01')
      expect(instances[0].get(:config)).to eq('name' => 'dns01', 'nameserver' => '8.8.8.8')
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = Puppet::Type.type(:opn_haproxy_resolver).new(
        name: 'dns01@opnsense01',
        ensure: :present,
      )
      resources = { 'dns01@opnsense01' => resource }

      described_class.prefetch(resources)
      expect(resource.provider.name).to eq('dns01@opnsense01')
    end
  end
  ...
  describe '#create' do
    let(:resource) do
      Puppet::Type.type(:opn_haproxy_resolver).new(
        name: 'dns01@opnsense01',
        ensure: :present,
    ...
    it 'sends correct POST to add endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/addresolver',
        { 'resolver' => { 'nameserver' => '8.8.8.8', 'name' => 'dns01' } },
  ...
  describe '#destroy' do
    ...
    it 'sends correct POST to del endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/delresolver/aaa-bbb', {}
  ...
  describe '#flush' do
    ...
    let(:provider) do
      described_class.new(
        ...
        device: 'opnsense01',
        uuid: 'aaa-bbb',
        config: { 'name' => 'dns01', 'nameserver' => '8.8.8.8' },
      )
    end
    ...
    it 'sends correct POST to set endpoint' do
      provider.config = { 'nameserver' => '1.1.1.1' }

      expect(client).to receive(:post).with(
        'haproxy/settings/setresolver/aaa-bbb',
        { 'resolver' => { 'nameserver' => '1.1.1.1', 'name' => 'dns01' } },
      ).and_return({ 'result' => 'saved' })
      allow(PuppetX::Opn::HaproxyReconfigure).to receive(:mark)
