...
describe Puppet::Type.type(:opn_user).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches users from the API' do
      ...
      expect(instances[0].name).to eq('admin@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('auth/user/search', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'name' => 'admin', 'email' => 'admin@example.com' }] })
      resource = type_class.new(name: 'admin@opnsense01')
      described_class.prefetch({ 'admin@opnsense01' => resource })
      expect(resource.provider.name).to eq('admin@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add endpoint' do
      resource = type_class.new(name: 'admin@opnsense01', config: { 'email' => 'admin@example.com' })
  ...
  describe '#destroy' do
    it 'calls the del endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'admin@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set endpoint when config has changed' do
      resource = type_class.new(name: 'admin@opnsense01', config: { 'email' => 'new@example.com' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'admin@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
