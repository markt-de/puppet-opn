...
require 'puppet/type/opn_haproxy_fcgi'
require 'puppet/provider/opn_haproxy_fcgi/opnsense_api'

RSpec.describe Puppet::Type.type(:opn_haproxy_fcgi).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'returns instances from the API' do
      allow(client).to receive(:post).with('haproxy/settings/search_fcgis', {}).and_return(
        'rows' => [
          { 'uuid' => 'aaa-bbb', 'name' => 'php-fpm', 'docroot' => '/var/www' },
        ],
      )
  ...
  describe '#create' do
    ...
    it 'sends correct POST to add endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/add_fcgi',
        { 'fcgi' => { 'docroot' => '/var/www', 'name' => 'php-fpm' } },
  ...
  describe '#destroy' do
    ...
    it 'sends correct POST to del endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/del_fcgi/aaa-bbb', {}
  ...
  describe '#flush' do
    ...
    it 'sends correct POST to set endpoint' do
      ...
      expect(client).to receive(:post).with(
        'haproxy/settings/set_fcgi/aaa-bbb',
        { 'fcgi' => { 'docroot' => '/srv/www', 'name' => 'php-fpm' } },
      ).and_return({ 'result' => 'saved' })
      allow(PuppetX::Opn::HaproxyReconfigure).to receive(:mark)
