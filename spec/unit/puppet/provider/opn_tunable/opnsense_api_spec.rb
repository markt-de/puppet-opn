...
describe Puppet::Type.type(:opn_tunable).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches tunables from the API' do
      ...
      expect(instances[0].name).to eq('net.inet.tcp.sendspace@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('core/tunables/search_item', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'tunable' => 'net.inet.tcp.sendspace', 'value' => '65536' }] })
      resource = type_class.new(name: 'net.inet.tcp.sendspace@opnsense01')
      described_class.prefetch({ 'net.inet.tcp.sendspace@opnsense01' => resource })
      expect(resource.provider.name).to eq('net.inet.tcp.sendspace@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add_item endpoint' do
      resource = type_class.new(name: 'net.inet.tcp.sendspace@opnsense01', config: { 'value' => '65536' })
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      allow(client).to receive(:post).with('core/tunables/add_item', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:tunable]).to receive(:mark).with('opnsense01', client)
      provider.create
    end
  end
  ...
  describe '#destroy' do
    it 'calls the del_item endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'net.inet.tcp.sendspace@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set_item endpoint when config has changed' do
      resource = type_class.new(name: 'net.inet.tcp.sendspace@opnsense01', config: { 'value' => '32768' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'net.inet.tcp.sendspace@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
  ...
  describe '.post_resource_eval' do
    it 'delegates to ServiceReconfigure[:tunable].run' do
      expect(PuppetX::Opn::ServiceReconfigure[:tunable]).to receive(:run)
