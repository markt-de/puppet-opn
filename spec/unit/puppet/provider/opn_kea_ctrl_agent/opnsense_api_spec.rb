...
require 'spec_helper'
require 'puppet/provider/opn_kea_ctrl_agent/opnsense_api'

describe Puppet::Type.type(:opn_kea_ctrl_agent).provider(:opnsense_api) do
  let(:provider_class) { described_class }
  let(:type_class) { Puppet::Type.type(:opn_kea_ctrl_agent) }
  ...
  describe '.instances' do
    it 'fetches settings via GET' do
      allow(client).to receive(:get).with('kea/ctrl_agent/get')
                                    .and_return({ 'ctrlagent' => { 'general' => { 'enabled' => '1', 'http_host' => '127.0.0.1' } } })
  ...
  describe '#create' do
    it 'saves settings via POST' do
      ...
      expect(client).to receive(:post).with(
        'kea/ctrl_agent/set',
        hash_including('ctrlagent' => { 'general' => { 'enabled' => '1' } }),
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      provider = described_class.new
      resource.provider = provider
      allow(client).to receive(:post).with('kea/ctrl_agent/set', anything)
                                     .and_return({ 'result' => 'saved' })
  ...
  describe '#destroy' do
    it 'saves empty settings and marks reconfigure via ServiceReconfigure' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'opnsense01', config: { 'general' => { 'enabled' => '1' } },
                                     })
      allow(client).to receive(:post).with('kea/ctrl_agent/set', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:kea]).to receive(:mark).with('opnsense01', client)
  ...
  describe '#flush' do
    it 'saves pending config via POST' do
      resource = type_class.new(name: 'opnsense01', config: { 'general' => { 'enabled' => '0' } })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'opnsense01', config: { 'general' => { 'enabled' => '1' } },
                                     })
      provider.instance_variable_set(:@pending_config, { 'general' => { 'enabled' => '0' } })
      expect(client).to receive(:post).with(
        'kea/ctrl_agent/set',
        hash_including('ctrlagent' => { 'general' => { 'enabled' => '0' } }),
      ).and_return({ 'result' => 'saved' })
      provider.flush
    end
