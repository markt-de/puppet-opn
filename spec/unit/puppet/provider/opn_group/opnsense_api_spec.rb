...
describe Puppet::Type.type(:opn_group).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches groups from the API' do
      ...
      expect(instances[0].name).to eq('admins@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      ...
      described_class.prefetch({ 'admins@opnsense01' => resource })
      expect(resource.provider.name).to eq('admins@opnsense01')
    end
  end
  ...
  describe '#destroy' do
    it 'calls the del endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'admins@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set endpoint when config has changed' do
      resource = type_class.new(name: 'admins@opnsense01', config: { 'description' => 'Super admins' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'admins@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
