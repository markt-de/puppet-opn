...
type_class = Puppet::Type.type(:opn_haproxy_user)
...
RSpec.describe provider_class do
  ...
  describe '.relation_fields' do
    it 'is defined and contains expected keys' do
      ...
      expect(fields.keys).to include('linkedActions')
    end
  end

  describe '.instances' do
    it 'returns an empty array when no users exist' do
      allow(client).to receive(:post).with('haproxy/settings/search_users', {})
                                     .and_return({ 'rows' => [] })
    ...
    it 'translates UUIDs to names' do
      allow(client).to receive(:post).with('haproxy/settings/search_users', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'name' => 'stats_user', 'linkedActions' => 'uuid1' }] })
      expect(PuppetX::Opn::HaproxyUuidResolver).to receive(:translate_to_names)
        .and_return({ 'name' => 'stats_user', 'linkedActions' => 'redirect_https' })

      instances = described_class.instances
      expect(instances.size).to eq(1)
      expect(instances.first.get(:name)).to eq('stats_user@opnsense01')
      expect(instances.first.get(:config)).to eq({ 'name' => 'stats_user', 'linkedActions' => 'redirect_https' })
    end
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      allow(client).to receive(:post).with('haproxy/settings/search_users', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa', 'name' => 'stats_user', 'password' => '$2b$hash' }] })
      allow(PuppetX::Opn::HaproxyUuidResolver).to receive(:translate_to_names)
        .and_return({ 'name' => 'stats_user', 'password' => '$2b$hash' })

      resource = type_class.new(name: 'stats_user@opnsense01', config: { 'password' => 'secret' })
      resources = { 'stats_user@opnsense01' => resource }
  ...
  describe '#exists?' do
    it 'returns true when ensure is present' do
      provider = described_class.new(ensure: :present, name: 'stats_user@opnsense01')
      expect(provider.exists?).to be true
    end

    it 'returns false when ensure is absent' do
      provider = described_class.new(ensure: :absent, name: 'stats_user@opnsense01')
  ...
  describe '#create' do
    it 'translates names to UUIDs before API call' do
      ...
      expect(PuppetX::Opn::HaproxyUuidResolver).to receive(:translate_to_uuids)
        .and_return({ 'name' => 'stats_user', 'password' => 'secret' })
      expect(PuppetX::Opn::HaproxyReconfigure).to receive(:mark).with('opnsense01', client)
      allow(client).to receive(:post).with('haproxy/settings/add_user', { 'user' => { 'name' => 'stats_user', 'password' => 'secret' } })
  ...
  describe '#destroy' do
    it 'deletes the user and marks reconfigure' do
      resource = type_class.new(name: 'stats_user@opnsense01')
      provider = described_class.new(
        ensure: :present,
        name: 'stats_user@opnsense01',
        device: 'opnsense01',
        uuid: 'aaa-bbb',
      )
      provider.resource = resource
      expect(PuppetX::Opn::HaproxyReconfigure).to receive(:mark).with('opnsense01', client)
      expect(client).to receive(:post).with('haproxy/settings/del_user/aaa-bbb', {})
  ...
  describe '#flush' do
    it 'translates names to UUIDs and updates the user' do
      resource = type_class.new(name: 'stats_user@opnsense01', config: { 'description' => 'Updated' })
      provider = described_class.new(
        ensure: :present,
        name: 'stats_user@opnsense01',
        device: 'opnsense01',
        uuid: 'aaa-bbb',
        config: { 'name' => 'stats_user', 'description' => 'Original' },
      )
      provider.resource = resource
      provider.config = { 'description' => 'Updated' }

      expect(PuppetX::Opn::HaproxyUuidResolver).to receive(:translate_to_uuids)
        .and_return({ 'name' => 'stats_user', 'description' => 'Updated' })
      expect(PuppetX::Opn::HaproxyReconfigure).to receive(:mark).with('opnsense01', client)
      allow(client).to receive(:post)
        .with('haproxy/settings/set_user/aaa-bbb', { 'user' => { 'name' => 'stats_user', 'description' => 'Updated' } })
        .and_return({ 'result' => 'saved' })
