...
require 'puppet/type/opn_haproxy_errorfile'
require 'puppet/provider/opn_haproxy_errorfile/opnsense_api'

RSpec.describe Puppet::Type.type(:opn_haproxy_errorfile).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'returns instances from the API' do
      allow(client).to receive(:post).with('haproxy/settings/search_errorfiles', {}).and_return(
        'rows' => [
          { 'uuid' => 'aaa-bbb', 'name' => 'err503', 'code' => '503' },
        ],
      )

      instances = described_class.instances
      expect(instances.length).to eq(1)
      expect(instances[0].name).to eq('err503@opnsense01')
      expect(instances[0].get(:config)).to eq('name' => 'err503', 'code' => '503')
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = Puppet::Type.type(:opn_haproxy_errorfile).new(
        name: 'err503@opnsense01',
        ensure: :present,
      )
      resources = { 'err503@opnsense01' => resource }

      described_class.prefetch(resources)
      expect(resource.provider.name).to eq('err503@opnsense01')
    end
  end
  ...
  describe '#create' do
    let(:resource) do
      Puppet::Type.type(:opn_haproxy_errorfile).new(
        name: 'err503@opnsense01',
        ensure: :present,
        config: { 'code' => '503' },
      )
    ...
    it 'sends correct POST to add endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/add_errorfile',
        { 'errorfile' => { 'code' => '503', 'name' => 'err503' } },
  ...
  describe '#destroy' do
    ...
    it 'sends correct POST to del endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/del_errorfile/aaa-bbb', {}
  ...
  describe '#flush' do
    ...
    it 'sends correct POST to set endpoint' do
      provider.config = { 'code' => '502' }

      expect(client).to receive(:post).with(
        'haproxy/settings/set_errorfile/aaa-bbb',
        { 'errorfile' => { 'code' => '502', 'name' => 'err503' } },
      ).and_return({ 'result' => 'saved' })
      allow(PuppetX::Opn::ServiceReconfigure[:haproxy]).to receive(:mark)
