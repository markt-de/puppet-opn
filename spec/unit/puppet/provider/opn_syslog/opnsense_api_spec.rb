...
describe Puppet::Type.type(:opn_syslog).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches syslog destinations from the API' do
      ...
      expect(instances[0].name).to eq('remote_syslog@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('syslog/settings/search_destinations', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'description' => 'remote_syslog', 'transport' => 'udp4' }] })
      resource = type_class.new(name: 'remote_syslog@opnsense01')
      described_class.prefetch({ 'remote_syslog@opnsense01' => resource })
      expect(resource.provider.name).to eq('remote_syslog@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add_destination endpoint' do
      resource = type_class.new(name: 'remote_syslog@opnsense01', config: { 'transport' => 'udp4' })
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      allow(client).to receive(:post).with('syslog/settings/add_destination', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:syslog]).to receive(:mark).with('opnsense01', client)
      provider.create
    end
  end
  ...
  describe '#destroy' do
    it 'calls the del_destination endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'remote_syslog@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set_destination endpoint when config has changed' do
      resource = type_class.new(name: 'remote_syslog@opnsense01', config: { 'transport' => 'tcp4' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'remote_syslog@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
  ...
  describe '.post_resource_eval' do
    it 'delegates to ServiceReconfigure[:syslog].run' do
      expect(PuppetX::Opn::ServiceReconfigure[:syslog]).to receive(:run)
