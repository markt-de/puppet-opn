...
require 'spec_helper'
require 'puppet/provider/opn_kea_dhcpv4/opnsense_api'

describe Puppet::Type.type(:opn_kea_dhcpv4).provider(:opnsense_api) do
  let(:provider_class) { described_class }
  let(:type_class) { Puppet::Type.type(:opn_kea_dhcpv4) }
  ...
  describe '.instances' do
    it 'fetches settings via GET' do
      allow(client).to receive(:get).with('kea/dhcpv4/get')
                                    .and_return({ 'dhcpv4' => { 'general' => { 'enabled' => '1' }, 'lexpire' => {}, 'ha' => {} } })
  ...
  describe '#destroy' do
    it 'saves empty settings and marks reconfigure' do
      ...
      allow(client).to receive(:post).with('kea/dhcpv4/set', anything)
                                     .and_return({ 'result' => 'saved' })
