...
require 'spec_helper'
require 'puppet/provider/opn_puppet_agent/opnsense_api'

describe Puppet::Type.type(:opn_puppet_agent).provider(:opnsense_api) do
  let(:provider_class) { described_class }
  let(:type_class) { Puppet::Type.type(:opn_puppet_agent) }
  ...
  describe '#create' do
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      provider = described_class.new
      resource.provider = provider
      allow(client).to receive(:post).with('puppetagent/settings/set', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:puppet_agent]).to receive(:mark).with('opnsense01', client)
      provider.create
    end
  ...
  describe '.post_resource_eval' do
    it 'delegates to ServiceReconfigure[:puppet_agent].run' do
      expect(PuppetX::Opn::ServiceReconfigure[:puppet_agent]).to receive(:run)
