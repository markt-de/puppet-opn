...
RSpec.describe provider_class do
  ...
  describe '.instances' do
    ...
    it 'translates UUIDs to names' do
      ...
      expect(instances.first.get(:name)).to eq('web_backend@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = type_class.new(name: 'web_backend@opnsense01', config: { 'mode' => 'http' })
      resources = { 'web_backend@opnsense01' => resource }
  ...
  describe '#exists?' do
    it 'returns true when ensure is present' do
      provider = described_class.new(ensure: :present, name: 'web_backend@opnsense01')
      expect(provider.exists?).to be true
    end

    it 'returns false when ensure is absent' do
      provider = described_class.new(ensure: :absent, name: 'web_backend@opnsense01')
  ...
  describe '#destroy' do
    it 'deletes the backend and marks reconfigure' do
      resource = type_class.new(name: 'web_backend@opnsense01')
  ...
  describe '#flush' do
    it 'translates names to UUIDs and updates the backend' do
      resource = type_class.new(name: 'web_backend@opnsense01', config: { 'mode' => 'tcp' })
      provider = described_class.new(
        ensure: :present,
        name: 'web_backend@opnsense01',
        device: 'opnsense01',
