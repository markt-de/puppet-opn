...
require 'spec_helper'
require 'puppet/provider/opn_kea_dhcpv4_peer/opnsense_api'

describe Puppet::Type.type(:opn_kea_dhcpv4_peer).provider(:opnsense_api) do
  let(:provider_class) { described_class }
  let(:type_class) { Puppet::Type.type(:opn_kea_dhcpv4_peer) }
  ...
  describe '.instances' do
    it 'fetches peers via search-only pattern' do
      allow(client).to receive(:post).with('kea/dhcpv4/searchPeer', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'name' => 'primary-node', 'role' => 'primary', 'url' => 'http://10.0.0.1:8000' }] })
    ...
    it 'skips rows with empty name' do
      allow(client).to receive(:post).with('kea/dhcpv4/searchPeer', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa', 'name' => '', 'role' => 'primary' }] })
      instances = described_class.instances
  ...
  describe '#create' do
    it 'calls the addPeer endpoint with name injected' do
      resource = type_class.new(name: 'primary-node@opnsense01', config: { 'role' => 'primary', 'url' => 'http://10.0.0.1:8000' })
  ...
  describe '#destroy' do
    it 'calls the delPeer endpoint' do
      ...
      expect(client).to receive(:post).with('kea/dhcpv4/delPeer/aaa-bbb', {})
