...
RSpec.describe provider_class do
  ...
  describe '.relation_fields' do
    it 'is defined and contains expected keys' do
      ...
      expect(fields.keys).to include(
        'defaultBackend', 'linkedActions', 'linkedErrorfiles',
        'basicAuthUsers', 'basicAuthGroups', 'linkedCpuAffinityRules',
        'linkedServers', 'linkedFcgi', 'linkedResolver',
        'healthCheck', 'linkedMailer',
        'ssl_certificates', 'ssl_default_certificate',
        'ssl_clientAuthCAs', 'ssl_clientAuthCRLs'
      )
    end
  end

  describe '.instances' do
    it 'returns an empty array when no frontends exist' do
    ...
    it 'translates UUIDs to names' do
      ...
      expect(instances.first.get(:name)).to eq('http_frontend@fw01')
      expect(instances.first.get(:config)).to eq({ 'name' => 'http_frontend', 'defaultBackend' => 'web_backend' })
    end
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = type_class.new(name: 'http_frontend@fw01', config: { 'mode' => 'http' })
      resources = { 'http_frontend@fw01' => resource }
  ...
  describe '#exists?' do
    it 'returns true when ensure is present' do
      provider = described_class.new(ensure: :present, name: 'http_frontend@fw01')
      expect(provider.exists?).to be true
    end

    it 'returns false when ensure is absent' do
      provider = described_class.new(ensure: :absent, name: 'http_frontend@fw01')
  ...
  describe '#destroy' do
    it 'deletes the frontend and marks reconfigure' do
      resource = type_class.new(name: 'http_frontend@fw01')
      provider = described_class.new(
        ensure: :present,
        name: 'http_frontend@fw01',
        device: 'fw01',
        uuid: 'aaa-bbb',
      )
      provider.resource = resource
      expect(PuppetX::Opn::HaproxyReconfigure).to receive(:mark).with('fw01', client)
      expect(client).to receive(:post).with('haproxy/settings/del_frontend/aaa-bbb', {})
  ...
  describe '#flush' do
    it 'translates names to UUIDs and updates the frontend' do
      resource = type_class.new(name: 'http_frontend@fw01', config: { 'mode' => 'tcp' })
      provider = described_class.new(
        ensure: :present,
        name: 'http_frontend@fw01',
        device: 'fw01',
        uuid: 'aaa-bbb',
        config: { 'name' => 'http_frontend', 'mode' => 'http' },
      )
      provider.resource = resource
      ...
      allow(client).to receive(:post)
        .with('haproxy/settings/set_frontend/aaa-bbb', { 'frontend' => { 'name' => 'http_frontend', 'mode' => 'tcp' } })
        .and_return({ 'result' => 'saved' })
