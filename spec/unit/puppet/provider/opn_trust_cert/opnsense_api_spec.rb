...
describe Puppet::Type.type(:opn_trust_cert).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches certificates from the API' do
      ...
      expect(instances[0].name).to eq('Web Server Cert@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('trust/cert/search', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'descr' => 'Web Server Cert', 'cert_type' => 'server' }] })
      resource = type_class.new(name: 'Web Server Cert@opnsense01')
      described_class.prefetch({ 'Web Server Cert@opnsense01' => resource })
      expect(resource.provider.name).to eq('Web Server Cert@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add endpoint' do
      resource = type_class.new(name: 'Web Server Cert@opnsense01', config: { 'cert_type' => 'server' })
  ...
  describe '#destroy' do
    it 'calls the del endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'Web Server Cert@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set endpoint when config has changed' do
      resource = type_class.new(name: 'Web Server Cert@opnsense01', config: { 'digest' => 'sha256' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'Web Server Cert@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
    ...
    it 'strips VOLATILE_FIELDS from config on flush' do
      resource = type_class.new(name: 'Web Server Cert@opnsense01', config: { 'digest' => 'sha256', 'key_type' => 'RSA' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'Web Server Cert@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
