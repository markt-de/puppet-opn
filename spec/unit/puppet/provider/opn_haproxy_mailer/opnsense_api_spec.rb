...
require 'puppet/type/opn_haproxy_mailer'
require 'puppet/provider/opn_haproxy_mailer/opnsense_api'

RSpec.describe Puppet::Type.type(:opn_haproxy_mailer).provider(:opnsense_api) do
  let(:provider_class) { described_class }
  let(:type_name) { :opn_haproxy_mailer }
  ...
  describe '.instances' do
    it 'returns instances from the API' do
      allow(client).to receive(:post).with('haproxy/settings/searchmailers', {}).and_return(
        'rows' => [
          { 'uuid' => 'aaa-bbb', 'name' => 'smtp01', 'mailserver' => 'mail.example.com' },
        ],
      )

      instances = described_class.instances
      expect(instances.length).to eq(1)
      expect(instances[0].name).to eq('smtp01@opnsense01')
      expect(instances[0].get(:config)).to eq('name' => 'smtp01', 'mailserver' => 'mail.example.com')
  ...
  describe '.prefetch' do
    it 'matches resources to instances' do
      ...
      resource = Puppet::Type.type(:opn_haproxy_mailer).new(
        name: 'smtp01@opnsense01',
        ensure: :present,
      )
      resources = { 'smtp01@opnsense01' => resource }

      described_class.prefetch(resources)
      expect(resource.provider.name).to eq('smtp01@opnsense01')
    end
  end
  ...
  describe '#create' do
    let(:resource) do
      Puppet::Type.type(:opn_haproxy_mailer).new(
        name: 'smtp01@opnsense01',
        ensure: :present,
        config: { 'mailserver' => 'mail.example.com' },
      )
    end
    ...
    it 'sends correct POST to add endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/addmailer',
        { 'mailer' => { 'mailserver' => 'mail.example.com', 'name' => 'smtp01' } },
  ...
  describe '#destroy' do
    ...
    it 'sends correct POST to del endpoint' do
      expect(client).to receive(:post).with(
        'haproxy/settings/delmailer/aaa-bbb', {}
  ...
  describe '#flush' do
    let(:resource) do
      Puppet::Type.type(:opn_haproxy_mailer).new(
        ...
        config: { 'mailserver' => 'smtp.example.com' },
      )
    end
    let(:provider) do
      described_class.new(
        ...
        device: 'opnsense01',
        uuid: 'aaa-bbb',
        config: { 'name' => 'smtp01', 'mailserver' => 'mail.example.com' },
      )
    end
    ...
    it 'sends correct POST to set endpoint' do
      provider.config = { 'mailserver' => 'smtp.example.com' }

      expect(client).to receive(:post).with(
        'haproxy/settings/setmailer/aaa-bbb',
        { 'mailer' => { 'mailserver' => 'smtp.example.com', 'name' => 'smtp01' } },
      ).and_return({ 'result' => 'saved' })
      allow(PuppetX::Opn::HaproxyReconfigure).to receive(:mark)
