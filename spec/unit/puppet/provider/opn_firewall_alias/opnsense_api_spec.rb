...
describe Puppet::Type.type(:opn_firewall_alias).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches aliases from the API' do
      ...
      expect(instances[0].name).to eq('my_alias@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('firewall/alias/search_item', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'name' => 'my_alias', 'type' => 'host' }] })
      resource = type_class.new(name: 'my_alias@opnsense01')
      described_class.prefetch({ 'my_alias@opnsense01' => resource })
      expect(resource.provider.name).to eq('my_alias@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add_item endpoint' do
      resource = type_class.new(name: 'my_alias@opnsense01', config: { 'type' => 'host' })
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      allow(client).to receive(:post).with('firewall/alias/add_item', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:firewall_alias]).to receive(:mark).with('opnsense01', client)
      provider.create
    end
  end
  ...
  describe '#destroy' do
    it 'calls the del_item endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'my_alias@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set_item endpoint when config has changed' do
      resource = type_class.new(name: 'my_alias@opnsense01', config: { 'type' => 'network' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'my_alias@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
  ...
  describe '.post_resource_eval' do
    it 'delegates to ServiceReconfigure[:firewall_alias].run' do
      expect(PuppetX::Opn::ServiceReconfigure[:firewall_alias]).to receive(:run)
