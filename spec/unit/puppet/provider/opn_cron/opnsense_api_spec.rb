...
describe Puppet::Type.type(:opn_cron).provider(:opnsense_api) do
  ...
  describe '.instances' do
    it 'fetches cron jobs from the API' do
      ...
      expect(instances[0].name).to eq('daily_reload@opnsense01')
  ...
  describe '.prefetch' do
    it 'matches instances to resources' do
      allow(client).to receive(:post).with('cron/settings/search_jobs', {})
                                     .and_return({ 'rows' => [{ 'uuid' => 'aaa-bbb', 'description' => 'daily_reload', 'command' => 'haproxy reload' }] })
      resource = type_class.new(name: 'daily_reload@opnsense01')
      described_class.prefetch({ 'daily_reload@opnsense01' => resource })
      expect(resource.provider.name).to eq('daily_reload@opnsense01')
    end
  end

  describe '#create' do
    it 'calls the add_job endpoint' do
      resource = type_class.new(name: 'daily_reload@opnsense01', config: { 'command' => 'haproxy reload' })
    ...
    it 'marks device for reconfigure via ServiceReconfigure' do
      ...
      allow(client).to receive(:post).with('cron/settings/add_job', anything)
                                     .and_return({ 'result' => 'saved' })
      expect(PuppetX::Opn::ServiceReconfigure[:cron]).to receive(:mark).with('opnsense01', client)
      provider.create
    end
  end
  ...
  describe '#destroy' do
    it 'calls the del_job endpoint' do
      ...
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'daily_reload@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
                                     })
  ...
  describe '#flush' do
    it 'calls the set_job endpoint when config has changed' do
      resource = type_class.new(name: 'daily_reload@opnsense01', config: { 'command' => 'firmware check' })
      provider = described_class.new
      resource.provider = provider
      provider.instance_variable_set(:@property_hash, {
                                       ensure: :present, name: 'daily_reload@opnsense01', device: 'opnsense01', uuid: 'aaa-bbb',
  ...
  describe '.post_resource_eval' do
    it 'delegates to ServiceReconfigure[:cron].run' do
      expect(PuppetX::Opn::ServiceReconfigure[:cron]).to receive(:run)
